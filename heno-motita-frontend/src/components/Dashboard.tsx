...
function Dashboard({ accessToken, user, onUnauthorized }: DashboardProps) {
  ...
  return <section className="dashboard">
    <nav className="dashboard-nav" aria-label="Módulos administrativos">{(['overview', 'managers', 'crews', 'students'] as Section[]).map((item) => <button key={item} className={section === item ? 'active' : ''} type="button" onClick={() => setSection(item)}>{{ overview: 'Resumen', managers: 'Encargados', crews: 'Cuadrillas', students: 'Alumnos' }[item]}</button>)}</nav>
    <div className="dashboard-heading"><div><p className="eyebrow">Administración</p><h2>{section === 'overview' ? 'Vista general' : { managers: 'Encargados', crews: 'Cuadrillas', students: 'Historial de alumnos' }[section]}</h2></div><div className="dashboard-actions">{section === 'managers' && <button type="button" className="dashboard-create-button" onClick={() => setManagerFormOpen(true)}>Registrar encargado</button>}{section === 'crews' && <button type="button" className="dashboard-create-button" onClick={() => setCrewFormOpen(true)} disabled={!managers.length}>Registrar cuadrilla</button>}{section === 'students' && <button type="button" className="dashboard-create-button" onClick={() => setStudentFormOpen(true)} disabled={!crews.length}>Registrar alumno</button>}<button type="button" className="refresh-button" onClick={() => void loadData()} disabled={loading}>Actualizar</button></div></div>
