...
function Dashboard({ accessToken, user, onUnauthorized }: DashboardProps) {
  ...
  return <section className="dashboard">
    ...
    <div className="dashboard-heading"><div><p className="eyebrow">Administración</p><h2>{section === 'overview' ? 'Vista general' : { managers: 'Encargados', crews: 'Cuadrillas', students: 'Historial de alumnos' }[section]}</h2></div><button type="button" className="refresh-button" onClick={() => void loadData()} disabled={loading}>Actualizar</button></div>
...
interface ContentProps {
  section: Section; managers: Manager[]; crews: Crew[]; students: StudentHistoryItem[]; managerForm: typeof defaultManager; setManagerForm: (value: typeof defaultManager) => void; creatingManager: boolean; onCreateManager: (event: FormEvent<HTMLFormElement>) => Promise<void>; crewForm: CrewInput; setCrewForm: (value: CrewInput) => void; showCrewForm: boolean; setShowCrewForm: (value: boolean) => void; creatingCrew: boolean; onCreateCrew: (event: FormEvent<HTMLFormElement>) => Promise<void>; studentForm: StudentInput; setStudentForm: (value: StudentInput) => void; studentCrewId: string; setStudentCrewId: (value: string) => void; showStudentForm: boolean; setShowStudentForm: (value: boolean) => void; creatingStudent: boolean; onCreateStudent: (event: FormEvent<HTMLFormElement>) => Promise<void>; updatingStatus: string; onManagerStatusChange: (id: string, status: Status) => Promise<void>; onCrewStatusChange: (id: string, status: Status) => Promise<void>; onStudentStatusChange: (id: string, status: Status) => Promise<void>
}

function DashboardContent(props: ContentProps) {
  const { section, managers, crews, students, managerForm, setManagerForm, creatingManager, onCreateManager, crewForm, setCrewForm, showCrewForm, setShowCrewForm, creatingCrew, onCreateCrew, studentForm, setStudentForm, studentCrewId, setStudentCrewId, showStudentForm, setShowStudentForm, creatingStudent, onCreateStudent, updatingStatus, onManagerStatusChange, onCrewStatusChange, onStudentStatusChange } = props
