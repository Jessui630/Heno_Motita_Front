...
import { listCrews } from "../api/adminApi";
import { ApiError } from "../api/httpClient";
import {
  getStudentCurrentCrew,
  listManagerCurrentCrews,
} from "../api/portalApi";
import type {
  Observation,
  ObservationImage,
...
const emptyObservation: ObservationInput = {
  lowerThirdScore: 0,
  middleThirdScore: 0,
  upperThirdScore: 0,
  notes: "",
...
function FieldWorkspace({
  ...
  >([]);
  const [loadedCrewId, setLoadedCrewId] = useState("");
    ...
    async function loadPortalCrews() {
      try {
        ...
        if (user.role === "CREW_MANAGER") {
          ...
          setAvailableCrews(
            data.crews.map(({ crew }) => ({ id: crew.id, name: crew.name })),
          );
        }
        if (user.role === "SUPER_ADMIN") {
          const data = await listCrews(accessToken);
          setAvailableCrews(
            data.crews.map((crew) => ({ id: crew.id, name: crew.name })),
          );
        }
        ...
        if (requestError instanceof ApiError && requestError.status === 401)
          ...
          setError(
            "No tienes una cuadrilla vigente para registrar actividad de campo.",
          );
      }
    }
  ...
  useEffect(() => {
    ...
    if (workspace instanceof HTMLElement)
      workspace.hidden = crewId !== loadedCrewId;
  }, [crewId, loadedCrewId, trees.length]);
  ...
  async function loadTrees(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const data = await listTrees(accessToken, crewId.trim());
      setTrees(data.trees);
      setTree(null);
      setObservations([]);
      setObservation(null);
  ...
  async function saveTree(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...treeForm,
      code: treeForm.code.trim().toUpperCase().replace(/\s+/g, "-"),
    };
    if (
    ...
    try {
      const data = tree
        ? await updateTree(accessToken, tree.id, payload)
        : await createTree(accessToken, crewId.trim(), payload);
      setMessage(data.message);
      setTree(data.tree);
      setTreeForm(data.tree);
  ...
  async function selectTree(value: Tree) {
    ...
    setError("");
    try {
      const data = await listObservations(accessToken, value.id);
  ...
  async function saveTreeStatus(status: Tree["status"]) {
    if (!tree) return;
    setLoading(true);
    try {
      const data = await updateTreeStatus(accessToken, tree.id, status);
      setTree(data.tree);
      setTreeForm(data.tree);
      await refreshTrees();
      setMessage(data.message);
  ...
  async function saveObservation(event: FormEvent<HTMLFormElement>) {
    ...
    const observationDate = new Date(
      observationForm.observationDate,
    ).toISOString();
    const payload = { ...observationForm, observationDate };
    try {
      const data = observation
        ? await updateObservation(accessToken, observation.id, payload)
        : await createObservation(accessToken, tree.id, payload);
      setMessage(data.message);
      setObservation(data.observation);
      setObservationForm({
        ...data.observation.hawksworth,
        notes: data.observation.notes,
        observationDate: data.observation.observationDate.slice(0, 16),
        latitude: data.observation.latitude,
        longitude: data.observation.longitude,
      });
      const list = await listObservations(accessToken, tree.id);
  ...
  async function selectObservation(value: Observation) {
    ...
    setObservationForm({
      ...value.hawksworth,
      notes: value.notes,
      observationDate: value.observationDate.slice(0, 16),
    ...
    setError("");
    try {
      const data = await listObservationImages(accessToken, value.id);
  ...
  async function saveObservationStatus(status: Observation["status"]) {
    if (!observation) return;
    setLoading(true);
    try {
      const data = await updateObservationStatus(
        accessToken,
        observation.id,
        status,
      );
  ...
  async function uploadImage(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      setError(
        "La evidencia debe ser JPG, PNG o WEBP, pesar máximo 8 MB y tener una descripción de hasta 500 caracteres.",
      );
      return;
    }
    setLoading(true);
    setError("");
    try {
      const data = await uploadObservationImage(
        accessToken,
        observation.id,
        image,
        description,
    ...
    <section className="workspace">
      <div className="dashboard-heading">
        <div>
          ...
          <h2>Árboles y observaciones</h2>
        </div>
      </div>
      <p className="form-description">
        Selecciona una cuadrilla para consultar o registrar datos. La API aplica
        los permisos de tu rol.
      </p>
      {error && (
      ...
      <form className="inline-form" onSubmit={(event) => void loadTrees(event)}>
        <label>
          Cuadrilla
        ...
        <>
          ...
          <div className="workspace-grid">
            <div>
              {showTreeForm && (
                <form
                  className="compact-form field-modal field-tree-modal"
                  onSubmit={(event) => void saveTree(event)}
                >
                  <h3>{tree ? "Editar árbol" : "Registrar árbol"}</h3>
                  <button
                    type="button"
                    className="field-modal-close"
                    aria-label="Cerrar formulario de árbol"
                  ...
                  <label>
                    ...
                    <input
                      value={treeForm.code}
                      onChange={(event) =>
                        setTreeForm({ ...treeForm, code: event.target.value })
                      }
                      minLength={3}
                      maxLength={40}
                      required
                  ...
                  <label>
                    Ubicación
                    <input
                      value={treeForm.locationDescription}
                      onChange={(event) =>
                        setTreeForm({
                          ...treeForm,
                  ...
                  <button disabled={loading}>
                    {tree ? "Guardar árbol" : "Registrar árbol"}
                  </button>
                  {tree && (
                    <button
                      type="button"
                  ...
                  {tree && canManageStatus && (
                    <select
                      value={tree.status}
                      onChange={(event) =>
                        void saveTreeStatus(
                          event.target.value as Tree["status"],
                        )
                      }
                    >
              ...
              <h3 className="list-heading">Árboles</h3>
              {trees.map((item) => (
                <button
                  className="record-button"
                  type="button"
                  key={item.id}
                  onClick={() => void selectTree(item)}
                >
                  {item.code} · {item.commonName} <small>{item.status}</small>
                </button>
              ))}
            </div>
            {tree && (
              <div>
                {showObservationForm && (
                  <form
                    className="compact-form field-modal field-observation-modal"
                    onSubmit={(event) => void saveObservation(event)}
                  >
                    <h3>
                    ...
                    {(
                      ...
                      <label key={field}>
                        {
                          {
                            lowerThirdScore: "Tercio inferior",
                            middleThirdScore: "Tercio medio",
                            upperThirdScore: "Tercio superior",
                          }[field]
                        }
                        <select
                          value={observationForm[field]}
                          onChange={(event) =>
                            setObservationForm({
                              ...observationForm,
                              [field]: Number(event.target.value) as 0 | 1 | 2,
                            })
                          }
                        >
                    ...
                    {observation && canManageStatus && (
                      <select
                        value={observation.status}
                        onChange={(event) =>
                          void saveObservationStatus(
                            event.target.value as Observation["status"],
                          )
                        }
                      >
                ...
                <h3 className="list-heading">Observaciones</h3>
                {observations.map((item) => (
                  <button
                    className="record-button"
                    type="button"
                    key={item.id}
                    onClick={() => void selectObservation(item)}
                  >
                    Puntuación {item.hawksworth.totalScore}/6{" "}
                ...
                {observation && showEvidenceForm && (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      className="field-modal-close"
                      aria-label="Cerrar formulario de evidencia"
