...
export function updateStudentStatus(accessToken: string, studentId: string, status: 'ACTIVE' | 'INACTIVE' | 'BLOCKED') { return request<{ message: string; student: Student }>(`/students/${studentId}/status`, json(accessToken, 'PATCH', { status })) }
